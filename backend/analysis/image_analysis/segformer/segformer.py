...
def save_segmentation_image(seg_map: np.ndarray, filename: str):
    ...
    seg_img.save(os.path.join(OUTPUT_DIR, filename.replace(".", "_segformer.")))
...
def main():
    ...
    if len(args) == 2:
        ...
        try:
            ...
            cur.execute("ALTER TABLE image_analysis ADD COLUMN IF NOT EXISTS segformer_gvi NUMERIC;")
            cur.execute("SELECT segformer_gvi FROM image_analysis WHERE location_id = %s", (location_id,))
            ...
            value = gvi(full_path)
            cur.execute("""
                INSERT INTO image_analysis (location_id, image_name, segformer_gvi)
                VALUES (%s, %s, %s)
                ON CONFLICT (location_id)
                DO UPDATE SET segformer_gvi = EXCLUDED.segformer_gvi;
        ...
        for i in sorted(imgnames):
            ...
            try:
                ...
                loc_id = result[0]
                cur.execute("SELECT segformer_gvi FROM image_analysis WHERE location_id = %s", (loc_id,))
